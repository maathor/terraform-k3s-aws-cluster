...
resource "aws_lb" "server-lb" {
  name               = replace(substr("${local.name}-int-${random_pet.lb.id}", 0, 24), "/-$/", "")
  internal           = true
...
resource "aws_lb_target_group" "server-6443" {
  name     = replace(substr("${local.name}-6443-${random_pet.lb.id}", 0, 24), "/-$/", "")
  port     = 6443
  protocol = "TCP"
...
resource "aws_lb" "lb" {
  ...
  name               = replace(substr("${local.name}-ext-${random_pet.lb.id}", 0, 24), "/-$/", "")
  internal           = false
  load_balancer_type = "network"
...
resource "aws_lb_target_group" "agent-443" {
  count    = local.create_external_nlb
  name     = replace(substr("${local.name}-443-${random_pet.lb.id}", 0, 24), "/-$/", "")
  port     = 443
  protocol = "TCP"
  vpc_id   = data.aws_vpc.default.id
...
resource "aws_lb_target_group" "agent-80" {
  count    = local.create_external_nlb
  name     = replace(substr("${local.name}-80-${random_pet.lb.id}", 0, 24), "/-$/", "")
  port     = 80
  protocol = "TCP"
  vpc_id   = data.aws_vpc.default.id
